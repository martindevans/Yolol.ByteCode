...
namespace Benchmark
{
    public class FlexBench
    {
        ...
        private static void RunTestFile(FileInfo file)
        {
            ...
            var ast = Parse(txt);
            outerTimer.Stop();
            Console.WriteLine($" - Parse:\t{outerTimer.Elapsed.TotalMilliseconds} ms");

            // Compile the program
            var internalsMap = new InternalsMap();
            var externalsMap = new ExternalsMap();
            outerTimer.Start();
            var compiled = ast.Compile(internalsMap, externalsMap);
            outerTimer.Stop();

            // Write out time to compile program
            Console.WriteLine($" - Compile:\t{outerTimer.Elapsed.TotalMilliseconds} ms");

            // Set up runtime
            var externalsArr = new Value[externalsMap.Count];
            Array.Fill(externalsArr, Number.Zero);
            var internalsArr = new Value[internalsMap.Count];
            Array.Fill(internalsArr, Number.Zero);
            var outputIdx = externalsMap[new VariableName(":output")];
            var cpu = new CpuState(compiled, internalsArr, externalsArr, new Value[128]);
            ...
            // Verify final result
            var output = externalsArr[outputIdx];
            if (output != "ok")
