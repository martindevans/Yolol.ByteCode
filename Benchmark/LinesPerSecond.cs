...
namespace Benchmark
{
    public class LinesPerSecond
    {
        ...
        public LinesPerSecond()
        {
            ...
            timer.Start();
            _compiled = ast.Compile(_internalsMap, _externalsMap);
        ...
        public void Run()
        {
            ...
            while (true)
            {
                ...
                Console.WriteLine($"{lps:#,##0.00} l/s | {avg:#,##0.00} avg | {stdDev:#,##0.00} dev | o: {_externals[oidx]} | s:{Math.Min(samples.Count, c)}");
            }
        ...
        {
            var state = new CpuState(_compiled, _internals, _externals, new Value[128]);
